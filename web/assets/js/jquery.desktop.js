    ...
    init: {
      ...
      desktop: function() {
        ...
        d.on('dblclick', 'a.icon', function() {
            ...
            if (!$('div.window').hasClass('window_full')) {
                JQD.util.window_resize('a.window_resize');
            }
          ...
          d.on('click', 'a.iconShortcut', function() {
        ...
        d.on('dblclick', 'div.window_top', function() {
           $('a.window_min').closest('div.window').hide();
        });
...
var isOpen = false;
$(function(){
    $("#bar_top").dblclick(function(){
        if(!isOpen){
            var height = $('body').height()-$("#bar_bottom").height()-$("#bar_top").height()-7;
            var div=$("#bar_top_header");
            div.animate({height:height,opacity:'0.5'},"slow");
            isOpen = true;
        } else {
            var div=$("#bar_top_header");
            div.animate({height:'0%',opacity:'0.5'},"slow");
            isOpen = false;
        }
    });
    $("#bar_top_inner").click(function(){
        if(!isOpen){
            var height = $('body').height()-$("#bar_bottom").height()-$("#bar_top").height()-7;
            var div=$("#bar_top_header");
            div.animate({height:height,opacity:'0.5'},"slow");
    ...
    $(window).resize(function(){
        var count = parseInt(Math.abs($("#desktop").find('a.abs').size()/parseInt((Math.abs($("body").height()-30)/80))))+1;
        var count2 = parseInt((Math.abs($("body").height()-30)/80));
        var j=0;
        var m = 0;
        var z = 0;
        var top = 10;
        ...
        for(var i=0; i<count; i++){
            for(j=m; j<count2+m; j++){
                $($('#desktop').find('a.abs').eq(j)).css('top', top+z*80 );
                $($('#desktop').find('a.abs').eq(j)).css('left', left+i*90);
                z++;
            }
            m=j;
            j=j+1;
...
//        alert($('body').height());
        if($('body').height()<495){
//            alert($('body').height());
            $(".operation_button_container button").css('width', '40px');
        } else {
            $(".operation_button_container button").css('width', '90px')
        }
